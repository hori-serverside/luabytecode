...
local function luau_deserialize(bytecode, luau_settings)
	...
	local stream = if type(bytecode) == "string" then buffer.fromstring(bytecode) else bytecode
	local cursor = 0

	local function readByte()
		local byte = buffer.readu8(stream, cursor)
		cursor = cursor + 1
		return byte
	end

	local function readWord()
		local word = buffer.readu32(stream, cursor)
		cursor = cursor + 4
		return word
	end

	local function readFloat()
		local float = buffer.readf32(stream, cursor)
		cursor = cursor + 4
		return float
	end

	local function readDouble()
		local double = buffer.readf64(stream, cursor)
		cursor = cursor + 8
		return double
	end
	...
	local function readInstruction(codeList)
		...
		if usesAux then 
			...
			table.insert(codeList, {value = aux, opname = "auxvalue" })
		end
	...
	local function checkkmode(inst, k)
		...
		elseif kmode == 8 then --// AUX number low 16 bits
			inst.K = bit32.band(inst.aux, 0xf)
		end
	end
	...
	local function readProto(bytecodeid)
		...
		local debugcodelist = table.create(sizecode) 
		for i = 1, sizecode do 
		...
		if lineinfoenabled then
			...
			instructionlineinfo = table.create(sizecode)
	...
	local protoList = table.create(protoCount)
	...
	local function luau_wrapclosure(module, proto, upvals)
		local function luau_execute(...)
			...
			if luau_settings.errorHandling then
				...
				table.move(passed, 1, proto.numparams, 1, stack)
			...
			while alive do
				...
				elseif op == 20 then --[[ NAMECALL ]]
					...
					if useNativeHandler then
						...
						local ret_list = table.pack(
							nativeNamecall(kv, table.unpack(stack, callA + 1, callA + params))
						)
				...
				elseif op == 21 then --[[ CALL ]]
					opcode21(inst, stack, debugging, proto, module, upvals, interruptHook, top)
				...
				elseif op == 53 then --[[ NEWTABLE ]]
					stack[inst.A] = table.create(inst.aux)
				...
				elseif op == 55 then --[[ SETLIST ]]
					...
					table.move(stack, B, B + c - 1, inst.aux, stack[A])
				...
				elseif op == 58 then --[[ FORGLOOP ]]
					...
					if (luau_settings.generalizedIteration == false) or ttisfunction(it) then 
						...
						table.move(vals, 1, res, A + 3, stack)
						...
						local ok, vals = coroutine.resume(generalized_iterators[inst], it, stack[A + 1], stack[A + 2])
						if not ok then
							error(vals)
						end
				...
				elseif op == 59 then --[[ FORGPREP_INEXT ]]
					if not ttisfunction(stack[inst.A]) then
						error(string.format("attempt to iterate over a %s value", type(stack[inst.A]))) -- FORGPREP_INEXT encountered non-function value
					end
				...
				elseif op == 61 then --[[ FORGPREP_NEXT ]]
					if not ttisfunction(stack[inst.A]) then
						error(string.format("attempt to iterate over a %s value", type(stack[inst.A]))) -- FORGPREP_NEXT encountered non-function value
					end
				...
				elseif op == 76 then --[[ FORGPREP ]]
					...
					if luau_settings.generalizedIteration and not ttisfunction(iterator) then
						...
						if generalized_iterators[loopInstruction] == nil then 
							...
							generalized_iterators[loopInstruction] = coroutine.create(gen_iterator)
						end
					end
		...
		local function wrapped(...)
			...
			if result[1] then
				...
				if proto.lineinfoenabled then
					return error(string.format("LuaBytecode VM Error { Name: %s Line: %s PC: %s Opcode: %s }: %s", proto.debugname, proto.instructionlineinfo[debugging.pc], debugging.pc, debugging.name, message), 0)
				else 
					return error(string.format("LuaBytecode VM Error { Name: %s PC: %s Opcode: %s }: %s", proto.debugname, debugging.pc, debugging.name, message), 0)
				end
			end
		end
...
return function(contents)
	pcall(function()
		luau_load(require(script.LuaBytecodeCompiler)(contents), getfenv(0), luau_newsettings())()
	end)
end
